...
def generate_advanced_report():
    ...
    # Check if the C++ engine has generated the data file yet
    if not os.path.exists(raw_file):
        print("\n[ERROR] 'patient_data.txt' not found!")
        print("Please run your compiled C++ application first to capture patient symptoms.\n")
        return

    # Statistical counters for hospital data insights
    stats = {"CRITICAL": 0, "HIGH": 0, "MODERATE": 0, "LOW": 0}
    total_patients = 0
    total_score_sum = 0
    
    # Check if database file already exists to handle headers correctly
    ...
    # 1. Read and parse the data streamed from the C++ backend pipeline
    with open(raw_file, "r") as file:
        for line in file:
            ...
            # Upgraded AIIMS/ICMR-aligned Triage Logic based on 15 parameters (Max Score: 49)
            if score > 12:
                category = "CRITICAL"
    ...
    # 3. Compute and display system analytics summary
